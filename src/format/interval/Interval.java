...
/**
 * @author Shiwayari Yonohana Representation of Intervals consisting of
 *         Integers.
 */
public class Interval implements Comparable<Interval> {
	...
	/**
	 * @return True if the next (smaller or greater) value just outside of the
	 *         specified interval is equal to the smallest or greatest value in i.
	 *         Also returns false if intersects(i) returns true.
	 */
	...
	/**
	 * @return The intersection of the intervals or null if the intersection is
	 *         empty.
	 */
	...
	/**
	 * Unites this interval with the interval i, but only if the resulting union is
	 * also an interval and not a set, so that the union contains all values greater
	 * than its smallest and smaller than its greatest value.
	 * 
	 * @return True if the union was successful, which is only the case if either
	 *         intersects(i) or adjacentTo(i) returns true.
	 */
	...
	public TreeSet<Interval> subtraction(Interval i) {
		...
		if (containsLeft || containsRight) {
			// double checked because otherwise the case (containsLeft && containsRight) ==
			// true is not handled correctly in (*)
			if (!containsRight) {
	...
	/**
	 * @return a string representation of this interval in the format "[a,b]" where
