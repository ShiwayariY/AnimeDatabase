...
public class Date implements Comparable<Date> {
	...
	/**
	 * Checks if the given date exists. Day, month or year = 0 is used for unknown dates.
	...
	public Date nextDay() throws UnknownDateException {
		...
		try {
			date = new Date(d ,m, y);
		} catch (InvalidDateException e) {} //Never occurs due to the algorithm in this method.
		return date;
	}
	...
	public int elapsedDaysUntil(Date endDate) throws IllegalArgumentException, UnknownDateException {
		...
		int d = day, m = month, y = year;
		Date currDate = null;
		try { currDate = new Date(d, m, y); } catch (InvalidDateException e) {}	//Never occurs, because it is a copy of this object.
	...
	/**
	 * Compares two dates. A later date is considered as bigger.
	 * Unknown dates (date, month or year are 0) can also be compared.
	 */
