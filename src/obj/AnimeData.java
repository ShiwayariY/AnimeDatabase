...
public class AnimeData {
	...
	public static enum AirStatus {FINISHED, AIRING, SOON};
	...
	public static final String AnimeTemplate = "" +
			...
			"\n" + //can be "" or a list of "startDate - endDate" seperated by \n, where the dates
				   //are in the format Date.toString(). endDate can be "" (null is used), but not startDate
			"<HDD>\n" +
			"\n" + //can be "", in which case null is used
			"<Groups>\n" +
	...
	private static Vector<String> names = new Vector<String>(); //use the index of an anime in this vector to find data of this anime from the other vectors
	...
	private static Vector<TreeMap<Integer, RangeList>> groups = new Vector<TreeMap<Integer, RangeList>>();
	
	private static TreeMap<Integer, Group> groupList = new TreeMap<>();
	...
	public static void toFile(File saveFile) throws IOException {
		...
		for(int id : groupList.keySet()) {
			...
			for(; i < commentLines.length; i++) {
				...
				if(commentLines[i].length() > 0) { break; }
			}
			for(; i < commentLines.length; i++) {
	...
	public static void loadFile(File saveFile) throws IOException {
		names.clear(); sequels.clear(); episodes.clear(); comments.clear(); released.clear(); downloaded.clear();
		watched.clear(); airStatus.clear(); periods.clear(); hdds.clear(); groups.clear();
		...
		while(m.find()) {
			...
			if(seq.equals("")) { sequels.add(null);	} else { sequels.add(seq); }
			...
			if(hdd.equals("")) { hdds.add(null); } else { hdds.add(hdd); }
		...
		//saves IDs of constituents for each group; must be done because the constituent Groups must first be
		//created before adding them as instance of Group to other groups as constituents.
		...
		while(groupM.find()) {
			...
			while(nameM.find()) { g.addName(nameM.group(1)); }
			...
			while(constitM.find()) { constits.add(Integer.valueOf(constitM.group(1))); }
	...
	/**
	 * @return an unmodifyable SortedSet containing all anime names in ascending order.
	 */
	...
	/**
	 * @param anime
	 * @return a deep copy of the Map of all comments for this anime mapped to their episode intervals. 
	 */
	...
	/**
	 * @param anime
	 * @return a copy of the RangeList of released episodes of the given anime,
	 * so that no change to the RangeList affect the internal RangeList.
	...
	/**
	 * @param anime
	 * @return the number of episodes in the RangeList of released episodes or
	 * -1 if the anime does not exist.
	 */
	...
	/**
	 * @param anime
	 * @return a copy of the RangeList of downloaded episodes of the given anime,
	 * so that no change to the RangeList will affect the internal RangeList.
	 * If the anime does not exist, null is returned.
	 */
	...
	/**
	 * @param anime
	 * @return the number of episodes in the RangeList of downloaded episodes or
	 * -1 if the anime does not exist.
	 */
	...
	/**
	 * @param anime
	 * @return a copy of the RangeList of watched episodes of the given anime,
	 * so that no change to the RangeList will affect the internal RangeList.
	 * If the anime does not exist, null is returned.
	 */
	...
	/**
	 * @param anime
	 * @return the number of episodes in the RangeList of watched episodes or
	 * -1 if the anime does not exist.
	 */
	...
	/**
	 * @param anime
	 * @return the hdd of the given anime. If the anime does not exist, or has
	 * no hdd specified, null is returned.
	 */
	...
	/**
	 * @param anime
	 * @return a deep copy of the list of Periods of the given anime or null
	 * if the anime does not exist. Neither changes to the Vector nor to the
	 * contained Period objects will affect the internal Vector. The returned
	 * Vector will be sorted in ascending order.
	 */
	...
	/**
	 * @param anime
	 * @return the air status of the given anime
	 * or null if the anime does not exist.
	 */
	...
	/**
	 * @param anime
	 * @return a list of groups mapped to their subbed episodes
	 * of the given anime, or null if the anime does not exist.
	 * Neither changes to the Map nor to the contained RangeList
	 * objects affect the internal Map.  
	 */
	...
	/**
	 * @return a vector containing all existing groups sorted 
	 * case-insensitive alphabetically by their displayed name.
	 */
	...
	/**
	 * @param anime
	 * @return the name of the sequel of the given anime,
	 * or null if the anime does not exist or has no sequel.
	 */
	...
	/**
	 * @param anime
	 * @return the name of the prequel of the given anime,
	 * or null if the anime does not exist or has no prequel.
	 */
	...
	/**
	 * @param anime
	 * @return true if the given anime has a sequel, else false.
	 * Also returns false if the given anime does not exist.
	 */
	...
	/**
	 * @param anime
	 * @return a vector containing all anime related to the given anime in
	 * ascending order (first, sequel, sequel2, ..). In general, the given
	 * anime will not be the first element of the resulting vector.
	 * Returns an empty vector if the given anime does not exist.
	 */
	...
	/**
	 * @param g
	 * @return the ID of the given group, or -1 if the group is not in the list of all groups.
	 */
	...
	/**
	 * Adds a * to the main windows title to indicate a
	 * possible change in data which has not yet been saved.
	 */
	public static void dataModified() {
		MainWindow w = MainWindow.thisWindow;
		String t = null;
		if(w != null) { t = w.getTitle(); }
	...
	/**
	 * Adds the given group to the group list, if it is not already in the list,
	 * and assigns an unused ID to it (ID is only used internally by this class).
	 * @param g
	 */
	...
	/**
	 * Deletes the given group from the group list, from each anime,
	 * and from each other groups that contain the given group as constituent.
	 ...
	 * @param newName
	 * @return true if anime did exist and newName did not
	 * exist before, i.e. the replacement was successful
	 */
	 ...
	 * @param anime
	 * @param sequel
	 * @return true if the sequel was set successfully, i.e. the anime
	 * and sequel exist and the sequel did not already have a prequel.
	 */
	...
	/**
	 * Set the number of episodes of the given anime.
	 * Set to 0 for unknown number of episodes.
	 * @param anime
	...
	//new implementation: comments
	/**
	 * Adds the given comment to the given episode interval
	 * of the given anime. If the interval already has a comment
	 * associated to it, it is overwritten.
	 * If the given comment is the empty string, the entry for
	 * the given interval is removed.
	 * @param anime
	 * @param epInt
	 * @param comment
	...
	//TODO remove inconsistent functionality ~ rangelist
	/**
	 * Set the watched episodes of the given anime to all episodes from 1 to wt.
	 * For wt = 0, the watched-RangeList is overwritten with an empty RangeList.
	 * @param anime
	...
	/**
	 * Set the HDD of the given anime. To remove the HDD
	 * from the given anime, use null for hdd.
	 * @param anime
	 * @param hdd
	 * @return true if the anime exists.
	 */
	...
	/**
	 * Adds a new period with the given startDate to the given anime, if
	 * the startDate does not already exist.
	 * @param anime
	 * @param startDate
	...
	/**
	 * Sets the end date of the period with the given start date of the given anime to
	 * finDate.
	 * @param anime
	 * @param startDate
	 * @param finDate
	 * @return true if the end date was set successfully, i.e. the anime and start
	 * date exist, and finDate is a valid end date for the existing start date.
	 */
	 ...
	 * @param anime
	 * @param date
	 * @return true if the period was successfully removed, i.e. the anime and period did exist.
	 */
	...
	/**
	 * Adds the interval [start, end] to the list of subbed episodes
	 * of the given group. If there is already an entry for the group,
	 * the new interval is united with all the existing intervals.
	 * @param anime
	 * @param groupID
	 * @param start
	 * @param end
	...
	/**
	 * Removes the interval [start, end] from the list of subbed episodes
	 * of the given group. If the group does not exist, do nothing. If no
	 * episodes are left after removing, the group is removed from the list.
	 * @param anime
	 * @param group
	 * @param start
	 * @param end
	...
	/**
	 * Adds the next biggest episode to the list of watched episodes, i.e.
	 * adds the integer which is equal to [the biggest integer in
	 * the watched-RangeList + 1] to the watched-RangeList. If the
	 * watched-RangeList is empty, the interval [1,1] is added.
	 * Does nothing if the given anime does not exist.
	 * @param anime
	...
	//TODO fix inconsistent functionality
		/**
		 * Adds the next biggest episode to the list of downloaded episodes, i.e.
		 * adds the integer which is equal to [the biggest integer in the
		 * downloaded-RangeList + 1] to the downloaded-RangeList. If the
		 * downloaded-RangeList is empty, the interval [1,1] is added.
		 * Does nothing if the given anime does not exist.
		 * @param anime
		 * @return true if the anime does exist.
		 */
