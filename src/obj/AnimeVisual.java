...
public class AnimeVisual implements ActionListener, KeyListener, MouseListener, FocusListener, OpensTextFieldWindow {
...
//	 ---------------------------------------- Constants ---------------------------------------- //
	...
	public void keyPressed(KeyEvent e) {
		if (e.getKeyCode() == KeyEvent.VK_ENTER && save.getActionCommand().equals("Save")) {
			...
			save.setIcon(editIcon);
			save();
		} else if (e.getSource() == data && e.getKeyCode() == KeyEvent.VK_PLUS) {
			AnimeData.incrementReleased(name);
	...
	private void removeGroup(int i) {
		try {
			...
			// NumberFormatException should not occur because only integers are allowed for
			// start, end
		}
	}
	...
	/**
	 * Adds value to the start(if x = 1) or end(if x = 2) of the interval at
	 * position y.
	 * 
	 * @param x
	 * @param y
	...
	public void focusGained(FocusEvent e) {
		data.setBackground(new Color(230, 230, 230));
	}
