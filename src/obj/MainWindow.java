...
public class MainWindow extends JFrame implements ActionListener, KeyListener, OpensTextFieldWindow {
	...
	public static enum AnimeSort {NAME, EP, STDATE, FINDATE};
	public static enum GroupSort {NAME, ANIME, EP}	//NAME sorts by displayed name,
													//ANIME by # of subbed animes, EP by # of subbed eps 
	public static enum Order {ASC, DESC};
	public static enum View {ANIME, TOWATCH, TODOWNLOAD, DATESWATCHED, DATESWATCHING, HDD, GROUPS};
	...
	private static JMenuItem[] aReg = { new JMenuItem("All"), new JMenuItem("A"), new JMenuItem("B"), new JMenuItem("C"), new JMenuItem("D"), new JMenuItem("E"),
										new JMenuItem("F"), new JMenuItem("G"), new JMenuItem("H"), new JMenuItem("I"), new JMenuItem("J"), new JMenuItem("K"),
										new JMenuItem("L"), new JMenuItem("M"), new JMenuItem("N"), new JMenuItem("O"), new JMenuItem("P"), new JMenuItem("Q"),
										new JMenuItem("R"), new JMenuItem("S"), new JMenuItem("T"), new JMenuItem("U"), new JMenuItem("V"), new JMenuItem("W"),
										new JMenuItem("X"), new JMenuItem("Y"), new JMenuItem("Z") };	
...
//		UIManager.getDefaults().put("ComboBox.selectionBackground", new ColorUIResource(BGCOLOR));
		...
		if(e.getActionCommand().equals("AlphabeticRegister")){
			...
			for (int j = 0; j < aReg.length; j++) {
				if(aReg[j].getText().equals(searchFilter)){
					if(j > 1) {
						aReg[j-1].setAccelerator(KeyStroke.getKeyStroke(KeyEvent.getExtendedKeyCodeForChar(','), KeyEvent.CTRL_DOWN_MASK));
					}
					if(j<26) {
						aReg[j+1].setAccelerator(KeyStroke.getKeyStroke(KeyEvent.getExtendedKeyCodeForChar('.'), KeyEvent.CTRL_DOWN_MASK));
					}
				...
				if(j == aReg.length - 1) {
					...
					aReg[aReg.length-1].setAccelerator(KeyStroke.getKeyStroke(KeyEvent.getExtendedKeyCodeForChar(','), KeyEvent.CTRL_DOWN_MASK));
				}
			}
			refreshView();
		}
		...
		if(sortOption == AnimeSort.STDATE || sortOption == AnimeSort.FINDATE) {
			...
			for(String animeName : nameList) {
				...
				if(hasNoDate) {
					...
					} catch (InvalidDateException e) {} //never occurs since 00.00.0000 is valid
				}
			}
			sortedAnimes = new Vector<String>();
			for(TreeSet<String> set : sorted.values()) {
	 ...
	 * @param sortOption
	 * @param orderOption
	 * @return a Vector containing all groups, where the ordering is specified
	 * by sortOption and orderOption.
	 */
	...
	/**
	 * @return a Vector containing the names of those anime, that pass
	 * the currently set search filter. The name-filter is applied case-insensitive.
	 * The ordering is specified by the currently set sort and order settings.
	 */
	private static Vector<String> searchAnimes() {
		...
		while(!sorted.isEmpty()) {
			...
			if(	(					
					(searchBeginOnly && toProcess.toLowerCase().startsWith(searchFilter.toLowerCase())) ||
	...
	/**
	 * @return a Vector containing the groups that pass the currently
	 * set search filter. The name-filter is applied case-insensitive.
	 * The ordering is specified by the currently set sort and order
	 * settings.
	 */
	private static Vector<Group> searchGroups() {
		...
		while(!sorted.isEmpty()) {
			...
			if(
					(searchBeginOnly && currName.toLowerCase().startsWith(searchFilter.toLowerCase())) ||
	...
	/**
	 * Constructs the "ANIME" view on the 'main' JPanel.
	 * Only the animes in the given Vector are used, and
	 * are used in the order they appear in the Vector.
	 * @param container
	 * @param animes
	 */
	private static void makeAnimeView(Vector<String> animes) {
		while(!animes.isEmpty()) {
			...
			main.add(row);
			for(String s : AnimeData.relations(animes.firstElement())) {
				row.add(AnimeData.visual(s));
				animes.remove(s);
			}
		}
	}
	...
	/**
	 * Constructs a list view, i.e. either "TOWATCH" or "TODOWNLOAD"
	 * on the 'main' JPanel. Only the animes in the given Vector
	 * are used, and are used in the order they appear in the Vector.
	 * 
	 * Calling this method when the current view is neither "TOWATCH"
	 * nor "TODOWNLOAD" produces undefined results.
	 * @param container
	 * @param animes
	 */
	...
	/**
	 * Constructs a date view, i.e. either "DATESWATCHED" or "DATESWATCHING"
	 * on the 'main' JPanel. Only the animes in the given Vector
	 * are used, and are used in the order they appear in the Vector.
	 * 
	 * Calling this method when the current view is neither "DATESWATCHED"
	 * nor "DATESWATCHING" produces undefined results.
	 * @param container
	 * @param animes
	 */
	private static void makeDateView(Vector<String> animes) {
		...
		for(String toProcess : animes) {
			...
			} catch(NoSuchElementException e) {} //no periods exist -> anime not yet watched
			...
			if(
				lastEntry != null && (
				(view == View.DATESWATCHED && watched) ||
				(view == View.DATESWATCHING && !watched))					
			) {
				count++;
	...
	/**
	 * Constructs the "HDD" view on the 'main' JPanel.
	 * Only the animes in the given Vector are used, and
	 * are used in the order they appear in the Vector.
	 * @param container
	 * @param animes
	...
	/**
	 * Constructs the "GROUP" view on the 'main' JPanel.
	 * Only the groups in the given Vector are used, and
	 * are used in the order they appear in the Vector.
	 * @param container
	 * @param groups
	 */
	private static void makeGroupView(Vector<Group> groups) {
		...
		for(final Group toProcess : groups) {
			...
			JLabel[] data = {nameL,
					new JLabel(Integer.toString(animeNum)),
					new JLabel(Integer.toString(epNum))
			};
	...
	/**
	 * This method should only be called by the makeGroupView method.
	 * Opens a GroupDialog to view and/or modify the group.
	 */
	private static void modifyGroup(Group g) {
		...
		if(d.getStatus()) {
			...
			for(String name : newNames) { g.addName(name); } //<-- does nothing if a name is already in oldNames
			oldNames.removeAll(newNames);
			for(String name : oldNames) { g.removeName(name); }
	...
	/**
	 * This method should only be called be the makeGroupView method.
	 * Opens a window showing all animes subbed by the given group.
	 * @param g
	 */
	...
	/**
	 * This method should only be called be the makeGroupView method.
	 * Opens a window showing all alliances in which this group participates.
	 * @param g
	 */
